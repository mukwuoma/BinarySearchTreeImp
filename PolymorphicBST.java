...
public class PolymorphicBST<K extends Comparable<K>, V> {
	...
	/**
	 * Returns a subtree containing the values fromKey-toKey. It will include
	 * fromKey and toKey if they are found in the original tree. The values for
	 * fromKey and toKey do not actually need to be in the tree. You can assume than
	 * fromKey is less than or equal to toKey. The new tree does not need to
	 * preserve the shape of the original subtree. Your implementation must be
	 * efficient avoiding unnecessary travesals of the tree. Also you MAY not add an
	 * auxiliary method to support the implementation of this method.
	 * 
	 * @return PolymorphicBST consisting of subset of SearchTreeMap
	 */
	public PolymorphicBST<K, V> subMap(K fromKey, K toKey) {
		PolymorphicBST<K, V> newTree = new PolymorphicBST<>();
		newTree.root = root.subTree(fromKey, toKey);
		return newTree;
	}
